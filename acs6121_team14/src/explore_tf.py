...
# all the pos is in the odom frame
...
class Explore:
    ...
    def __init__(self):
        ...
        # subscribe to odom: after set up start_time
        self.odom_sub = rospy.Subscriber("odom", Odometry, self.odom_callback)
    ...
    def posToId(self, x, y):# (x,y) in odom, Id in map occupacy map
        point_odom=PointStamped()
        point_odom.header.frame_id = "odom"
        point_odom.header.stamp =rospy.Time(0)
        point_odom.point.x=x
        point_odom.point.y=y
        point_odom.point.z=0.0
        point_map=self.tf_listener.transformPoint('map', point_odom)
        col=math.floor((point_map.point.x+10)/0.05)
        row=math.floor((point_map.point.y+10)/0.05) # x=column * resolution + origin_x
    ...
    def sampleInIdn(self, idn): # idn in odom, change self.x, self.y in map
        row=idn/3
        col=idn%3
    ...
    def map_callback(self, data):
        ...
        if not self.isEnd:
            if self.completion: # availiable for sending
                ...
                while valid is False: # resample till valid
                    ...
                    for n in candidates:
                        ...
                        self.x=self.numToRandPos(col_n+1)
                        self.y=self.numToRandPos(row_n+1)
                        idp=self.posToId(self.x, self.y) # (x,y) in odom, return Id in map
                        self.map = data.data[idp] #should be integer
            ...
            if time.perf_counter()-self.goal_set_time>self.time_out:
                idn=self.posToIdn(self.x, self.y) # (X,y) in odom, return idn in odom
    ...
    def set_goal(self):
        ...
        point_odom.header.frame_id = "odom"
        point_odom.header.stamp =rospy.Time(0)
        point_odom.point.x=self.x
        point_odom.point.y=self.y
        point_odom.point.z=0.0
        point_map=self.tf_listener.transformPoint('map', point_odom)
